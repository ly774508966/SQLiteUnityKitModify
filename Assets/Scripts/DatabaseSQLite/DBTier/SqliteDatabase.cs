...
//		Debug.Log ("DB path:" + pathDB);
	...
	private IntPtr Prepare (string query)
	{
		...
		if (sqlite3_prepare_v2 (_connection, query, query.Length, out stmHandle, IntPtr.Zero) != SQLITE_OK) {
			...
			string message = string.Format("error:{0}, sql:{1}", Marshal.PtrToStringAnsi (errorMsg), query);
			PlayerPrefs.SetString("SqliteException", message);
